...
function RemoveBackground() {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      fd.append("image", input); // backend me multer ya equivalent "image" field expect kar raha hai

      const { data } = await axios.post("/api/ai/remove-image-background", fd, {
        headers: {
    ...
    <div className="flex flex-col md:flex-row gap-6 w-full mx-auto px-4 py-8">
      ...
      >
        ...
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-4">Supported formats: .jpg, .png, .jpeg</p>
        </div>
        ...
        !content ? (
          <div className="flex-1 bg-white border border-gray-200 rounded-xl shadow-sm p-6 flex items-center justify-center text-center">
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-600">Click the "Remove background" button to write your article here.</p>
            </div>
          </div>
        ) : (
